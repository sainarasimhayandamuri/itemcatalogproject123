...
@application.route('/catalog/items/')
def home():
...
def gconnect():
    ...
    if request.args.get('state') != login_session['state']:
        king = make_response(json.dumps('Invalid state parameter.'), 401)
        king.headers['Content-Type'] = 'application/json'
        return king
    ...
    if result.get('error') is not None:
        king = make_response(json.dumps(result.get('error')), 500)
        king.headers['Content-Type'] = 'application/json'
        return king
...
def gdisconnect():
    ...
    if result['status'] == '200':
        king = make_response(json.dumps('Successfully disconnected.'), 200)
        king.headers['Content-Type'] = 'application/json'
        return king
    else:
        king = make_response(
...
# Add a new category.
@application.route("/catalog/category/new/", methods=['GET', 'POST'])
def add_category():
    """Add a new category."""
...
# Create a new item.
@application.route("/catalog/item/new/", methods=['GET', 'POST'])
def add_item():
    """Create a new item.
...
@application.route("/catalog/category/<int:category_id>/item/new/",
                   methods=['GET', 'POST'])
...
@application.route('/catalog/item/<int:item_id>/')
...
# Edit existing item.
@application.route("/catalog/item/<int:item_id>/edit/",
                   methods=['GET', 'POST'])
...
@application.route("/catalog/item/<int:item_id>/delete/",
                   methods=['GET', 'POST'])
...
@application.route('/catalog/category/<int:category_id>/items/')
...
# Edit a category.
@application.route('/catalog/category/<int:category_id>/edit/',
                   methods=['GET', 'POST'])
...
# Delete a category.
@application.route('/catalog/category/<int:category_id>/delete/',
                   methods=['GET', 'POST'])
...
@application.route('/api/v1/catalog.json')
...
@application.route('/api/v1/categories/JSON')
def categories_json():
